...
class World(object):
    ...
    def human_reward(self, trajs=None, lanes=None, roads=None, fences=None, speed=1., speed_import=1.):
        ...
        if trajs is None:
            ...
            trajs = [c.linear for c in self.cars if c!=trajs]
        r = 0.1*feature.control()
        theta = [207.45344712, -43.22037785, 33.0465513, 0.85747694, -60.04716053] # Learned Weights - Attempt 1
...
def world10():
    ...
    # world.fences += [clane.shifted(3), clane.shifted(-3)]
    ...
    # world.cars.append(car.NestedOptimizerCar(dyn, [0.3, 0.3, math.pi/2., 0.3], color='white'))

    world.cars.append(car.SimpleOptimizerCar(dyn, [0., 0.3, math.pi/2., 0.3], color='white'))
    world.cars.append(car.SimpleOptimizerCar(dyn, [0.15, 0.2, math.pi/2., 0.3], color='gray'))
    world.cars.append(car.SimpleOptimizerCar(dyn, [0.3, 0.2, math.pi/2., 0.3], color='red'))
    world.cars.append(car.SimpleOptimizerCar(dyn, [0.3, -0.2, math.pi/2., 0.3], color='orange'))
    world.cars.append(car.SimpleOptimizerCar(dyn, [0., -0.3, math.pi/2., 0.3], color='yellow'))
